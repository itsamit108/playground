...
Follow the [Next.js instructions](https://nextjs.org/docs) on how to create a new project. Then follow the steps in the [Installation guide](../installation) to install React Strict DOM.

> **Tip**
> Take a look at the [working example of Next.js with React Strict DOM on GitHub](https://github.com/facebook/react-strict-dom/tree/main/examples/nextjs).

## Babel configuration

Babel is not the default compiler when using Next.js App Router, but it can still be used. Install `babel-loader`. Then create a `babelLoader.config.js` (not `babel.config.js`) file as follows. This is used to optimize builds and enables static extraction of CSS for web. Learn how to configure the babel-preset in the [API docs](../api/babel-preset).
...
```js
// next.config.js (additions for platform-specific extensions)
